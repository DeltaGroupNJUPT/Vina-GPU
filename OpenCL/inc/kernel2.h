...
#define MAX_NUM_OF_ATOMS 200 // 这个应该是ligand的最大原子数？
...
#define MAX_NUM_OF_RANDOM_MAP 1000 // 设置得较大会出现stack overflow
#define GRIDS_SIZE 17 // 17张表
#define GRID_MI 65
#define GRID_MJ 71
#define GRID_MK 61
...
typedef struct { // 一定是按照深度优先搜索来存储
	...
	float	axis			[MAX_NUM_OF_RIGID][3]; // 第一行代表根节点，元素全为0
	float	relative_axis	[MAX_NUM_OF_RIGID][3]; // 第一行代表根节点，元素全为0
	float	relative_origin	[MAX_NUM_OF_RIGID][3]; // 第一行代表根节点，元素全为0
	
	int		parent			[MAX_NUM_OF_RIGID]; // 每个节点的父节点必须唯一！
